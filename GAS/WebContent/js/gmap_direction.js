...
function load() {
	if(geocoder==null){
     if (GBrowserIsCompatible()) {
		...
		gasIcon = new GIcon();
	    gasIcon.image= "/GAS/img/gmap_consegna.png";
	    gasIcon.shadow="/GAS/img/gmap_fiamma_ombra.png";
	    ...
	    contadiniIcon.image= "/GAS/img/gmap_fiamma.png";
	    contadiniIcon.shadow="/GAS/img/gmap_fiamma_ombra.png";
				...
				if(prima_cybercontadino==0){
					...
					if(to_place[idmark]==null){
						...
						if(count_punti_di_consegna==0){							
							solo_punti_di_consegna=0; //non si pu� aggiungere un punto di consegna
				...
				if(solo_punti_di_consegna==0){
					if(from_place[idmark]==null){
						...
						if(count_contadini==0){
							prima_cybercontadino=0; //non si pu� aggiungere un punto di consegna
							controlForm();
	...
	if(prima_cybercontadino > 0){
		if(solo_punti_di_consegna > 0){
			...
			if(ultimo_controllo > 0){
				document.getElementById('status_itinerario').innerHTML = "<img src=\"/GAS/img/valid.png\" /> Ora puoi creare l'itinerario";
				...
				document.getElementById('status_itinerario').innerHTML = "<img src=\"/GAS/img/notyet_valid.png\" /> Ora seleziona la data di partenza e poi crea l'itinerario";
				document.getElementById('crea_itinerario_pulsante').style.display = "none";
			}
		}else {
			//arancione
			document.getElementById('status_itinerario').innerHTML = "<img src=\"/GAS/img/notyet_valid.png\" /> Adesso seleziona almeno un punto di consegna per poter creare un nuovo itinerario";
			document.getElementById('crea_itinerario_pulsante').style.display = "none";
		}
	} else {
		//rosso
		document.getElementById('status_itinerario').innerHTML = "<img src=\"/GAS/img/not_valid.png\" /> Seleziona almeno un cybercontadino ed un punto di consegna per poter creare un nuovo itinerario";
		document.getElementById('crea_itinerario_pulsante').style.display = "none";
